...
public class DesktopBuildEngine {
    ...
    private int extractTDP(Component component) {
        ...
        // Use the helper methods for CPU and GPU
        if (component instanceof com.example.androidofpcsense.models.CPU) {
            return ((com.example.androidofpcsense.models.CPU)component).getTdpValue();
        }

        if (component instanceof com.example.androidofpcsense.models.GPU) {
            return ((com.example.androidofpcsense.models.GPU)component).getTdpValue();
        }

        // Fallback: try to extract from spec string
        if (component.getSpec() == null) return 0;
